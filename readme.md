...
桌面端为三栏版式：左侧导航栏、中间内容区、右侧信息栏，三块玻璃面板各自带圆角悬浮在背景光之上；窄屏下侧边栏自动收起为圆角抽屉，右侧栏折叠隐藏。
...
- **⚡ 全网飙升速报** —— 实时捕捉各平台热度急速飙升的焦点事件与动态排名
- **☁️ 全网热词云** —— 提取全网热搜高频关键词，支持一键点击联动过滤榜单
...
| 🎯 趣味内容 | 段子、冷笑话、一言、节日文案、答案之书、2048 小游戏、电子木鱼、每日壁纸、JS 题目、梗百科                       |
...
- **🔍 智能检索与快捷搜源** — 顶栏搜索框支持站内高频热词筛选、常用数据源直达（60s早报、天气、油价等）与必应/谷歌全网搜索，桌面自适应双行防挤压
- **⏱️ 万年历时钟胶囊** — 秒级走时，一日进度动态铺满；点击可呼出毛玻璃万年历弹窗（公历/农历/节假日/节气/调休速览）
- **🌤️ 本地天气** — 按访客 IP 定位城市，天空底色随天气与昼夜切换
- **🖼️ 必应壁纸** — 每日背景，自适应清晰度，支持 1080P / 4K 下载与往期浏览
- **✨ 流动光背景** — 壁纸关闭时（默认）以大块柔光作底，光块缓慢漂移，玻璃卡片透出底色
...
- **🔗 友情链接** — 侧栏底部版权行入口（作者主页 / 数据来源），样式随主题自适应
...
| `pnpm test`                         | 单元测试（归一化打分 / 热度口径 / 参数钳制）   |
| `pnpm run build:frontend`           | 构建前端压缩产物到 `public/dist/` 并校验新鲜度 |
| `pnpm run sync:version`             | 把 package.json 版本号同步进 app.js / sw.js    |
